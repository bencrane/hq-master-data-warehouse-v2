...
Uses Gemini to analyze a company's pricing page and determine if an annual commitment is required.
...
Returns:
{
  "annual_commitment_required": "yes" | "no" | "unclear",
...
def infer_annual_commitment(request: dict) -> dict:
    import requests
    ...
    try:
        ...
        raw_insert = (
            supabase.schema("raw")
            .from_("annual_commitment_required_payloads")
            .insert({
                "domain": domain,
        ...
        prompt = f"""Analyze this pricing page content and determine if an annual commitment is required.
...
Only return the JSON, nothing else."""
        ...
        try:
            ...
            annual_commitment_required = result.get("annual_commitment_required", "").lower()
            explanation = result.get("explanation", "")
        except json.JSONDecodeError:
            annual_commitment_required = "unknown"
            explanation = response_text

        valid_values = ["yes", "no", "unclear"]
        if annual_commitment_required not in valid_values:
            annual_commitment_required = "unknown"

        # 4. Insert into extracted
        supabase.schema("extracted").from_("company_annual_commitment_required").insert({
            "raw_payload_id": raw_payload_id,
            "domain": domain,
            "pricing_page_url": pricing_page_url,
            "annual_commitment_required": annual_commitment_required,
            "explanation": explanation,
        }).execute()

        # 5. Upsert into core
        supabase.schema("core").from_("company_annual_commitment_required").upsert({
            "domain": domain,
            "annual_commitment_required": annual_commitment_required,
            "explanation": explanation,
            "last_checked_at": "now()",
