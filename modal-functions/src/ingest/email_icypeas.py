"""
Icypeas Email Ingestion

Ingest endpoint for Icypeas email lookup results.
...
from extraction.email_icypeas import extract_email_icypeas
...
def ingest_email_icypeas(request: IcypeasRequest) -> dict:
    """
    Ingest Icypeas email lookup results.

    Stores raw payload, extracts to extracted.email_icypeas,
    ...
    try:
        ...
        raw_record = {
            ...
            "icypeas_raw_payload": request.icypeas_raw_payload,
        }
        ...
        extraction_result = extract_email_icypeas(
            supabase=supabase,
            ...
            company_name=request.company_name,
            company_linkedin_url=request.company_linkedin_url,
            icypeas_payload=request.icypeas_raw_payload,
        )

        # Get email from payload for response
        email = request.icypeas_raw_payload.get("email")
