"""
Compare Job Titles CSV Endpoint

Same as compare_job_titles but accepts comma-separated string instead of array.
For easy use with Clay when you have a CSV column.

Expects:
{
  "candidate_title": "VP of Engineering",
  "job_titles_csv": "CTO, Head of Engineering, Director of Engineering"
}
"""
...
class CompareJobTitlesCsvRequest(BaseModel):
    candidate_title: str
    job_titles_csv: str  # Comma-separated string
...
def compare_job_titles_csv(request: CompareJobTitlesCsvRequest) -> dict:
    """
    Compare a candidate job title against a comma-separated list of job titles.
    ...
    job_title_list = [t.strip() for t in request.job_titles_csv.split(",") if t.strip()]

    if not job_title_list:
        return {"success": False, "error": "job_titles_csv must contain at least one title"}
